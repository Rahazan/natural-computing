...
public class CompetitionEvaluator extends WindFarmLayoutEvaluator {
	...
	try {
	    ...
	    this.turbineFitnesses = new double[jsonTurbFit.length()];
	    
	    for (int i=0; i<jsonTurbFit.length(); i++) {
	    	  this.turbineFitnesses[i] = jsonTurbFit.getJSONArray(i).getDouble(0);
	    	  //System.out.println(this.turbineFitnesses[i]);
		}
